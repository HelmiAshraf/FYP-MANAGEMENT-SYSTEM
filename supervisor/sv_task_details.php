...
if (isset($_GET['task_id'])) {
    ...
    try {
        ...
        $sql = "SELECT task_title, task_description, task_date_due, task_sv_id
                FROM task 
...
?>
            ...
            <div id="editModal" class="modal fixed inset-0 z-50 flex items-center justify-center overflow-x-hidden overflow-y-auto transform scale-0 opacity-0">
                ...
                <div class="bg-white w-96 rounded-lg shadow-lg p-6 transform scale-100 opacity-100 transition-transform duration-300">
                    ...
                    <form action="function/task_form_controller.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label for="new_data" class="block mb-2 text-sm font-medium">New Data</label>
                        ...
                        <div class="mb-4">
                            <label for="doc_description" class="block mb-2 text-sm font-medium">Document Description</label>
                            <textarea name="task_description" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-100 border-gray-300 placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500" maxlength="300"><?php echo $task_description; ?></textarea>
            ...
            <div class="flex justify-between items-center">
                ...
                <div>
                    <a href="#" class="user_edit text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none block">Update task</a>
                </div>
            </div>
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-4 bg-gray-800 text-gray-400">
                <ul class="flex text-sm">
                    ...
                    <li>
                        <p class="mb-3 text-base text-gray-200"><?php echo $task_title; ?></p>
                    </li>
                </ul>
                ...
                <ul class="text-sm">
                    ...
                    <li>
                        <p class="mb-3 text-base text-gray-200"><?php echo $task_date_due; ?></p>
                    </li>
                </ul>
                <ul class="text-sm">
                    <li class="mr-2">
                    ...
                    <li>
                        <p class="mb-3 text-base text-gray-200"><?php echo $task_description; ?></p>
                    </li>
                </ul>
                </ul>
                <ul class="text-sm">
                    <li class="mr-2">
                        <p class="text-base font-semibold text-white">Assign To:</p>
                    </li>
                    <?php
                    $sql = "SELECT t.task_id, s.st_name
            FROM task t
            ...
            WHERE t.task_id = $task_id;";
                    ...
                    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        $assignedStudent = $row['st_name'];
                    ?>
                        <li>
                            <p class="text-base text-gray-200"><?php echo $assignedStudent; ?></p>
                        </li>
                    <?php
                    }
                    ?>
              ...
              FROM file_path f
              WHERE f.type_id = :task_id AND f.file_type = 'tsv'";
...
WHERE
su.supervisor_id = ? AND ts.task_id = ?;
";
...
?>
<!-- // TODO TUNJUK SUBMITED TASK -->

<h1 class="text-3xl font-bold mb-4 mt-4">Submited Tasks</h1>
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()) { ?>
                    ...
                    <tr class='border-b bg-gray-800 border-gray-700 hover:bg-gray-900'>
                        <td scope='row' class='px-6 py-4 font-medium whitespace-nowrap text-black'>
                            ...
                            <script>
                                function openFileInNewTab(file_name, file_name_without_extension) {
                                    // Construct the file URL
                                    var fileUrl = '../fyp_lecturer/view_pdf.php//?url=' + encodeURIComponent('../file/task/' + file_name) + '&name=' + encodeURIComponent(file_name_without_extension);
                        ...
                        <td class='px-6 py-3 '><?php echo $row['student_name']; ?></td>
                        <td class='px-6 py-3'>
                            <?php
